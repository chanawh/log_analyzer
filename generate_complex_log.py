...
programs = [
    "/boot/kernel.amd64/kernel", "isi_papi_d", "isi_stats_d",
...
while dt <= end_date:
    for hour in range(0, 24):
        for minute in range(0, 60, random.choice([10, 15, 30])):
            current_dt = dt.replace(hour=hour, minute=minute, second=random.randint(0,59))
            dtstr = current_dt.strftime("%Y-%m-%dT%H:%M:%S.{:06d}+01:00".format(random.randint(0, 999999)))
            ...
            if program == "syslogd":
                ...
                line = f"{dtstr} {prio} {host} isi_papi_d[{pid}]: {action} [{log_level}]"
                ...
                pid = random.randint(100, 99999)
                sid = sid_template.format(random.randint(1000000000,9999999999), random.randint(100000000,999999999), random.randint(100000000,999999999), random.randint(10000,99999))
...
for i in range(30):  # More events, more dates
    dt = start_date + timedelta(days=random.randint(0, (end_date - start_date).days), hours=random.randint(0,23), minutes=random.randint(0,59))
    dtstr = dt.strftime("%Y-%m-%dT%H:%M:%S.{:06d}+01:00".format(random.randint(0, 999999)))
    host = random.choice(hosts)
    prio = "<1.4>"
    pid = random.randint(100,99999)
